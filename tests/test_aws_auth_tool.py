...
def describe_AwsAuthTool():
    def describe_is_authorized():
        ...
        def it_read_only_does_not_grant_read_write() -> None:
            tool = AwsAuthTool()
            tool.authorize("123456789012", Role.READ_ONLY)
            assert_that(tool.is_authorized("123456789012", Role.READ_WRITE)).is_false()

        def it_read_write_does_not_grant_read_only() -> None:
            tool = AwsAuthTool()
            tool.authorize("123456789012", Role.READ_WRITE)
            assert_that(tool.is_authorized("123456789012", Role.READ_ONLY)).is_false()
